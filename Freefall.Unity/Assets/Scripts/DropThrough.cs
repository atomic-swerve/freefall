...
public class DropThrough : MonoBehaviour {
	...
	public void ActivateDrop() {
		...
		RaycastHit2D[] hits = groundChecker.GetHits(LayerMask.NameToLayer("Passable Ground"));
	...
	public void HandleDropInput() {
		if(Input.GetAxis("Y-Axis") < 0 && Input.GetButtonDown("A") && groundChecker.CheckGrounded(LayerMask.NameToLayer("Passable Ground"))) {
			ActivateDrop();
		}
	}
